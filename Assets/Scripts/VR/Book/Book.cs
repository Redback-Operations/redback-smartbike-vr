...
public class Book : MonoBehaviour
{
    ...
    void Update()
    {
        if (XRSettings.enabled && Interactor != null)
        {
            if (Interactor.TryGetCurrentUIRaycastResult(out RaycastResult location))
                InteractionPosition = location.screenPosition;
        }
        else
        {
            InteractionPosition = Input.mousePosition;
    ...
    {
        f = Vector3.Lerp(f, transformPoint(InteractionPosition), Time.deltaTime * 10);
    ...
    public void DragRightPageToPoint(Vector3 point)
    {
        ...
        Left.sprite = (currentPage < bookPages.Length) ? GetSprite(currentPage) : background;
        Left.transform.SetAsFirstSibling();

        Right.gameObject.SetActive(true);
        Right.transform.position = RightNext.transform.position;
        Right.transform.eulerAngles = new Vector3(0, 0, 0);
        Right.sprite = (currentPage < bookPages.Length - 1) ? GetSprite(currentPage + 1) : background;

        RightNext.sprite = (currentPage < bookPages.Length - 2) ? GetSprite(currentPage + 2) : background;
    ...
    public void DragLeftPageToPoint(Vector3 point)
    {
        ...
        Left.transform.eulerAngles = new Vector3(0, 0, 0);
        Left.sprite = (currentPage >= 2) ? GetSprite(currentPage - 2) : background;

        LeftNext.sprite = (currentPage >= 3) ? GetSprite(currentPage - 3) : background;
    ...
    {
        if (interactable)
            DragLeftPageToPoint(transformPoint(InteractionPosition));
    ...
    void UpdateSprites()
    {
        LeftNext.sprite = (currentPage > 0 && currentPage <= bookPages.Length) ? GetSprite(currentPage - 1) : background;
        RightNext.sprite = (currentPage >= 0 && currentPage < bookPages.Length) ? GetSprite(currentPage) : background;
    ...
    Sprite GetSprite(int index)
    {
        ...
        var main = page.Sprite.texture;
        var overlay = page.State == PageState.Selected ? selected.texture : locked.texture;
    ...
    public void OnMouseSelectLeftPage()
    {
        // cannot select left page when current page is 0
        if (currentPage == 0)
            return;
    ...
    {
        // cannot select right page when current page is more than the page count
    ...
    void ShowSelection()
    {
        ...
        Debug.Log($"Current Page: {currentPage} Selected Page: {SelectedPage}");

        if (SelectedPage != currentPage && SelectedPage + 1 != currentPage)
            return;

        Debug.Log($"Showing {(SelectedPage % 2 == 0 ? "Right" : "Left")} Selected Page: {SelectedPage}");

        if (SelectedPage % 2 == 0)
        {
            SelectRight?.gameObject.SetActive(true);
            SelectRight?.transform.SetAsLastSibling();
        }
        else
        {
            SelectLeft?.gameObject.SetActive(true);
            SelectLeft?.transform.SetAsLastSibling();
        }
    }
}
